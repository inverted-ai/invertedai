...
class ScenePlotter():
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.dynamic_traffic_lights = None
    ...
    def initialize_recording(
        ...
        if map_image is not None:
            self.dynamic_map_images = [map_image]

        if static_actors is not None:
            self.dynamic_traffic_lights = [{stat_act.actor_id: stat_act for stat_act in static_actors if stat_act.agent_type == 'traffic_light'}]
    ...
    def record_step(
        ...
        if map_image is not None:
            self.dynamic_map_images.append(map_image)

        if static_actors is not None:
            self.dynamic_traffic_lights.append({stat_act.actor_id: stat_act for stat_act in static_actors if stat_act.agent_type == 'traffic_light'})
    ...
    def _update_frame_to(self, frame_idx):
        if self.dynamic_map_images is not None:
            self.current_ax.imshow(self.dynamic_map_images[frame_idx], extent=self.extent)
    ...
    def _plot_traffic_light(
        ...
        if self.dynamic_traffic_lights is not None:
            light = self.dynamic_traffic_lights[frame_idx][light_id]
        else:
