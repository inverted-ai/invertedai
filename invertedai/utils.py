        ...
        if self.numbers is not None and agent_idx in self.numbers:
            if agent_idx not in self.box_labels:
                self.box_labels[agent_idx] = self.current_ax.text(
                    ...
                    ha='center',
                    va='center',
                    fontsize=18*self._dpi_scale* (250/self.fov),
                )
        ...
        if wps is not None and wps: #Ensure list is not empty
            ...
            if agent_idx not in self.waypoint_markers:
                ...
                self.waypoint_markers[agent_idx]["marker"] = self.current_ax.plot(
                    ...
                    marker=marker_data,
                    color='saddlebrown',
                    markersize=17.0*self._dpi_scale * (250/self.fov), # multiply by scaling factor relative to fov
                    linestyle='None',
                    zorder=6
                )[0]
                self.waypoint_markers[agent_idx]["text"] = self.current_ax.text(
                    ...
                    ha='center',
                    va='center',
                    fontsize=18*self._dpi_scale * (250/self.fov), # multiply by scaling factor relative to fov
                    zorder=6
                )
