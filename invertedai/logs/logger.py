...
class ScenarioV2LogWriter(LogWriter):
    def __init__(self):
        super().__init__()
    ...
    def export_to_file(
        ...
        for i, prop in enumerate(scenario_log.agent_properties):
            ...
            predetermined_agents_dict[str(i)] = {
                "entity_type": "vehicle" if prop.agent_type == "car" else prop.agent_type,
                "static_attributes": {
                    "length": prop.length,
                    "width": prop.width,
                    "rear_axis_offset": prop.rear_axis_offset,
            ...
            if remove_ego_states and i == 0:
                for i in range(1, len(scenario_log.agent_states) - 1):
                    del states_dict[str(i)]
