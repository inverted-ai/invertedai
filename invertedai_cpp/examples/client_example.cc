...
int main(int argc, char **argv) {
  try {
    ...
    LocationInfoRequest loc_info_req("{\"location\": \"" + location +
                                     "\", "
                                     "\"include_map_source\": true}");
    LocationInfoResponse loc_info_res = invertedai::location_info(loc_info_req, &session);

    auto image = cv::imdecode(loc_info_res.birdview_image_, cv::IMREAD_COLOR);
    ...
    InitializeRequest init_req(read_file("examples/initialize_body.json"));
    init_req.set_location(location);
    init_req.set_agent_num(agent_num);
    InitializeResponse init_res = invertedai::initialize(init_req, &session);

    DriveRequest drive_req(read_file("examples/drive_body.json"));
    drive_req.update(init_res);

    for (int t = 0; t < timestep; t++) {
      DriveResponse drive_res = invertedai::drive(drive_req, &session);
      auto image = cv::imdecode(drive_res.birdview_, cv::IMREAD_COLOR);
      cv::cvtColor(image, image, cv::COLOR_BGR2RGB);
      video.write(image);
