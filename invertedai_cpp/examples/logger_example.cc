...
int main(int argc, char **argv) {
    try {
        ...
        logger.set_loc_request(loc_info_req);
        logger.set_loc_response(loc_info_res);
        ...
        logger.set_init_request(init_req);
        logger.set_init_response(init_res);
        ...
        for (int t = 0; t < timestep; t++) {
            ...
            logger.append_drive_request(drive_req);
            logger.append_drive_response(drive_res);
        ...
        logger.write_log_to_file("test_output.json");
        video.release();
