...
auto validate_regions_100x100 (const std::vector<invertedai::Region>& regs,
                               ...
                               const std::map<AgentType, int>& agent_count_dict)
    {
        ...
        int spawn_x = region.center.x ;          // small horizontal offset
        int spawn_y = region.center.y +100;                // right through road centerline
...
// bazel build //large:large_main 
    ...
    std::mt19937 gen(rd());
    int initialize_seed = 3; //std::uniform_int_distribution<>(1, 1000000)(gen); // or fixed for repeatability 8003
    ...
    // --- Save
    cv::imwrite("test case3.png", stitched);
    std::cout << "Saved test case.png (" << stitched.cols << "x" << stitched.rows << ")\n";
    std::cout << "All done!\n";
    return 0;
}
