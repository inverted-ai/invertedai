...
def convert_to_pygame_coords(x, y):
    ...
    pygame_y = int((y_max - y) * H / y_range)
...
simulation = iai.Simulation(location=args.location, center=PreSets.map_centers[args.center], width=map_width, height=map_height,
                            agent_per_region=agent_per_region, screen=screen, convertor=convert_to_pygame_coords)
fps = 100
run = True
...
while run:
    ...
    screen.blit(pygame.transform.scale(pygame.transform.rotate(map_image, -90), (x_scale, y_scale)), top_left)
    ...
    # quadtree = QuadTree(NODE_CAPACITY, boundary, convertors=(convert_to_pygame_coords, convert_to_pygame_scales))
