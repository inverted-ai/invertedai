...
# create KeyedAgents 
agents = KeyedAgents(num_agents=num_agents)
# initialize the simulation by spawning NPCs and pass in keyed_agents
response = iai.initialize(location=location, keyed_agents=agents, random_seed=seed)
...
for _ in range(simulation_length):  # how many simulation steps to execute (10 steps is 1 second)
    ...
    scene_plotter.record_step(
        agent_states=response.agent_states,
        agent_properties=agents.get_properties(), #This is important to capture the new waypoints every time step
...
gif_name = f'{seed}_keyed_waypoint_example.gif'
