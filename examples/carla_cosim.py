...
import os
import time
import uuid  # NEW: needed for generating unique agent IDs for external agents
import carla
import argparse
import logging
import math
...
# OLD: called iai.get_regions_default + iai.large_initialize directly, returned flat agent_data list
# NEW: delegates to SimulationManager which handles regions, initialize, waypoints, and logging internally
#      ego + CARLA agents are passed as external_agent_data; IAI agents are managed internally
#      returns simulation_manager + iai_carla_actors dict instead of flat agent_data list
#
...
#     agent_data = assign_carla_blueprints_to_agents(world=world, vehicle_blueprints=vehicle_blueprints, agent_data=agent_data)
#     agent_states, agent_properties, agent_recurrent_states = [], [], []
...
def initialize_simulation(
    ...
    vehicle_blueprints,
    existing_agent_data,
    existing_agent_ids,         # NEW: stable string IDs for ego + CARLA ped agents
    traffic_lights_states=None,
    iai_log_path=None,          # NEW: if set, SimulationManager writes a JSON log to this path
):
    traffic_lights_states, carla2iai_tl = initialize_tl_states(world)
    ...
    # with actual CARLA bounding box dimensions and build iai_carla_actors dict (agent_id -> actor)
    ...
    # NEW return signature: simulation_manager + iai_carla_actors instead of flat agent_data list
...
def main():
    ...
    # NEW: generate output path early so it can be passed into SimulationManager via initialize_simulation
    ...
    # OLD: iai_log path was created after initialize_simulation; log_writer was set up separately
    # if args.iai_log:
    #     log_writer = iai.LogWriter()
    #     log_writer.initialize(location=args.location, location_info_response=location_info_response, init_response=response)
    ...
    # NEW: generate stable string IDs for ego + CARLA pedestrian agents (external to SimulationManager)
    existing_agent_ids = [str(uuid.uuid4()) for _ in range(len(agent_data))]

    # Initialize InvertedAI co-simulation
    # OLD: response, carla2iai_tl, location_info_response, agent_data = initialize_simulation(
    #          args=args, world=world, seed=args.seed, vehicle_blueprints=vehicle_blueprints,
    #          existing_agent_data=agent_data, traffic_lights_states=traffic_lights_states)
    # OLD: sim_agent_data = SimulationData(agent_data)
    # OLD: agent_properties = sim_agent_data.get_all_properties()
    # OLD: wp_manager = iai.WaypointManager(location_info_response=location_info_response,
    #          cfg=iai.WaypointManagerConfig(random_seed=args.seed, fail_soft=True))
    # NEW: SimulationManager returned instead; wp_manager and log_writer managed internally
    ...
    # sim_agent_data is kept only for CARLA actor references (interpolation, sensor attachment)
    # State tracking for external agents is done via cosim_agents, updated each step
    ...
    try:
        if args.capture_video:
            # NEW: ego CARLA actor now accessed via sim_agent_data (ego + peds only, index 0 is still ego)
            # OLD: actor_to_attach = sim_agent_data.all_agent_data[0].carla_actor (same, no change needed)
        ...
        for frame in tqdm(range(args.sim_length * int(1/IAI_TIME_STEP))):
            ...
            # Snapshot states before drive for interpolation
        ...
        if args.iai_log:
            ...
            # OLD: log_writer.export_to_file(log_path=f"{iai_output_data}.json")
            # NEW: SimulationManager.export_log() delegates to its internal LogWriter
            simulation_manager.export_log(path=f"{iai_output_data}.json")
            print(f"Generating birdview GIF.")
            # OLD: log_writer.visualize(...)
            # NEW: access the internal log_writer through SimulationManager for visualization
